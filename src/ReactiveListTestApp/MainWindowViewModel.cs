...
{
    /// <summary>The delay applied to standard reactive views.</summary>
    private const int ListViewThrottleMilliseconds = 0;

    /// <summary>The delay applied to secondary-index views.</summary>
    private const int IndexViewThrottleMilliseconds = 0;

    /// <summary>The maximum number of snapshots retained in the live tape.</summary>
    private const int TapeRetention = 800;

    /// <summary>The maximum number of snapshots retained by the indexed list.</summary>
    private const int IndexedRetention = 1_200;

    /// <summary>The maximum number of cache notifications retained in the stream inspector.</summary>
    private const int StreamRetention = 160;

    /// <summary>The name shared by the venue secondary indexes.</summary>
    private const string VenueIndexName = "ByVenue";

    /// <summary>The feature catalog category used by reactive view entries.</summary>
    private const string DataViewCategory = "Data view";

    /// <summary>The minimum producer rate selectable in the UI.</summary>
    private const int MinimumRate = 1_000;

    /// <summary>The maximum producer rate selectable in the UI.</summary>
    private const int MaximumRate = 100_000;

    /// <summary>The increment used when rounding a selected producer rate.</summary>
    private const int RateStep = 1_000;

    /// <summary>The number of projection frames produced each second.</summary>
    private const int ProjectionFramesPerSecond = 8;

    /// <summary>The minimum raw event count generated by a single-step request.</summary>
    private const int MinimumStepEvents = 100;

    /// <summary>The number of venue rows in the price matrix.</summary>
    private const int MatrixRowCount = 4;

    /// <summary>The number of instrument columns in each price matrix row.</summary>
    private const int MatrixColumnCount = 10;

    /// <summary>The interval between scalar metric updates.</summary>
    private const int MetricsSampleMilliseconds = 500;

    /// <summary>The initial capacity of each formatted matrix row.</summary>
    private const int MatrixTextCapacity = 80;

    /// <summary>The number of bytes in one kibibyte used by the allocation display.</summary>
    private const double BytesPerKilobyte = 1_024D;

    /// <summary>The producer that supplies raw market frames.</summary>
    private readonly LiveDataEngine _engine;

    /// <summary>The bounded reactive market tape.</summary>
    private readonly ReactiveList<InstrumentSnapshot> _tape = [];

    /// <summary>The current prices arranged by venue and instrument.</summary>
    private readonly Reactive2DList<double> _priceMatrix = CreatePriceMatrix();

    /// <summary>The retained snapshots that support named secondary indexes.</summary>
    private readonly QuaternaryList<InstrumentSnapshot> _indexedSnapshots = [];

    /// <summary>The latest snapshot for each instrument.</summary>
    private readonly QuaternaryDictionary<int, InstrumentSnapshot> _latestByInstrument = [];

    /// <summary>The dispatcher-owned matrix row projections.</summary>
    private readonly ReactiveList<MatrixRow> _matrixRows = [];

    /// <summary>The bounded collection event inspector.</summary>
    private readonly ReactiveList<StreamEvent> _streamEvents = [];

    /// <summary>The current predicate used by the dynamic tape view.</summary>
    private readonly BehaviorSignal<Func<InstrumentSnapshot, bool>> _dynamicFilter = new(static _ => true);

    /// <summary>The active venue keys used by the dynamic index views.</summary>
    private readonly BehaviorSignal<string[]> _venueKeys = new(["LSE"]);

    /// <summary>The stream subscriptions owned by the view model.</summary>
    private readonly List<IDisposable> _disposables = [];

    /// <summary>The dispatcher that owns all UI-facing projections.</summary>
    private readonly Dispatcher _dispatcher;

    /// <summary>The clock used to timestamp stream entries.</summary>
    private readonly TimeProvider _timeProvider;

    /// <summary>The stopwatch used to sample event and UI frame rates.</summary>
    private readonly Stopwatch _rateWindow = Stopwatch.StartNew();

    /// <summary>The stable event handler registered with the producer.</summary>
    private readonly EventHandler<MarketFrame> _frameHandler;

    /// <summary>The unfiltered tape view.</summary>
    private readonly FilteredReactiveView<InstrumentSnapshot> _allView;

    /// <summary>The alert-only tape view.</summary>
    private readonly FilteredReactiveView<InstrumentSnapshot> _alertsView;

    /// <summary>The user-controlled dynamic tape view.</summary>
    private readonly DynamicFilteredReactiveView<InstrumentSnapshot> _searchView;

    /// <summary>The latency-sorted tape view.</summary>
    private readonly SortedReactiveView<InstrumentSnapshot> _latencyView;

    /// <summary>The sector-grouped tape view.</summary>
    private readonly GroupedReactiveView<InstrumentSnapshot, string> _sectorView;

    /// <summary>The venue-filtered indexed list view.</summary>
    private readonly DynamicSecondaryIndexReactiveView<InstrumentSnapshot, string> _indexedVenueView;

    /// <summary>The venue-filtered indexed dictionary view.</summary>
    private readonly DynamicSecondaryIndexDictionaryReactiveView<int, InstrumentSnapshot> _dictionaryVenueView;

    /// <summary>The filtered collection event view.</summary>
    private readonly FilteredReactiveView<StreamEvent> _streamView;

    /// <summary>The raw event count accumulated in the current sampling window.</summary>
    private long _windowEvents;

    /// <summary>The raw event count observed during the previous sample.</summary>
    private long _lastWindowEvents;

    /// <summary>The number of UI frames accumulated in the current sample.</summary>
    private long _uiFrames;

    /// <summary>The next sequence number assigned to a stream entry.</summary>
    private long _streamSequence;

    /// <summary>Indicates whether continuous production is active.</summary>
    private bool _isRunning = true;

    /// <summary>The selected raw event rate.</summary>
    ...
    /// <summary>Initializes a new instance of the <see cref="MainWindowViewModel"/> class.</summary>
    /// <param name="timeProvider">The clock used by producer and stream timestamps.</param>
    internal MainWindowViewModel(TimeProvider timeProvider)
    {
        ...
        _latencyView = _tape.SortBy(
            static snapshot => snapshot.LatencyMilliseconds,
            descending: true,
            sequencer,
            ListViewThrottleMilliseconds);
        _sectorView = _tape.GroupBy(static snapshot => snapshot.Sector, sequencer, ListViewThrottleMilliseconds);
        _indexedVenueView = _indexedSnapshots.CreateDynamicViewBySecondaryIndex(
            VenueIndexName,
            _venueKeys,
            sequencer,
            IndexViewThrottleMilliseconds);
        ...
        _disposables.AddRange(CreateStreamSubscriptions());
    ...
    public System.Collections.IEnumerable MatrixRows => _matrixRows.Items;

    /// <summary>Gets the bounded cache notification log.</summary>
    public System.Collections.IEnumerable StreamEvents => _streamView.Items;

    /// <summary>Gets the feature catalog shown beside the stream inspector.</summary>
    public System.Collections.IEnumerable Features { get; }
...
= "LSE";
    ...
    /// <summary>Creates the static feature catalog shown by the application.</summary>
    /// <returns>The feature cards.</returns>
    private static FeatureCard[] CreateFeatureCatalog() =>
    [
        new(
            "ReactiveList<T>",
            "Reactive collection",
            "Thread-safe list with pooled batch mutations, versioned stream and activity collections.",
            "Bounded market tape using AddRange and RemoveRange."),
        new(
            "Reactive2DList<T>",
            "Reactive collection",
            "Observable rows plus inner Add, Insert, Set, Remove, Flatten and TotalCount operations.",
            "Four live venue lanes with ten instruments per row."),
        new(
            "QuaternaryList<T>",
            "Concurrent collection",
            "Four sharded writers with snapshots, batches, RemoveMany and named secondary indexes.",
            "Retained indexed ticks filtered by a dynamic venue signal."),
        new(
            "QuaternaryDictionary<TKey,TValue>",
            "Concurrent collection",
            "Sharded lookup map with AddOrUpdate, range edits and value secondary indexes.",
            "Latest quote per instrument with a dynamic venue index view."),
        new(
            "QuadList<T>",
            "Pooled hot path",
            "ArrayPool-backed non-reactive list optimized for spans and low-allocation batch work.",
            "Per-frame raw tick scratch buffer on the producer thread."),
        new(
            "QuadDictionary<TKey,TValue>",
            "Pooled hot path",
            "Pooled hash table with direct ref lookup and no notification overhead.",
            "Latest snapshot aggregation before reactive publication."),
        new(
            "Filtered / Dynamic views",
            DataViewCategory,
            "Read-only observable projections updated from source streams and changing predicates.",
            "All, alert-only and live text/alert query grids."),
        new(
            "Sorted / Grouped views",
            DataViewCategory,
            "Incrementally refreshed order and ReactiveGroup projections.",
            "Latency leaderboard and sector group summaries."),
        new(
            "Secondary-index views",
            DataViewCategory,
            "Static or signal-driven views that query named quaternary indexes.",
            "Venue selector drives list and dictionary views together."),
        new(
            "Connect / ChangeSet pipeline",
            "Stream",
            "Cache notifications bridge to ChangeSet transforms, filters, grouping and action callbacks.",
            "Bounded stream inspector shows real collection actions.")
    ];
    ...
    {
        var rows = new double[MatrixRowCount][];
        for (var row = 0; row < rows.Length; row++)
        {
            rows[row] = new double[MatrixColumnCount];
        }

        return new(rows);
    }

    /// <summary>Creates the subscriptions that feed the bounded stream inspector.</summary>
    /// <returns>The subscriptions owned by the view model.</returns>
    private IDisposable[] CreateStreamSubscriptions() =>
    [
        _tape.Stream.Subscribe(
            new ActionObserver<CacheNotify<InstrumentSnapshot>>(
                notification => RecordStream(
                    "ReactiveList",
                    notification.Action,
                    notification.Batch?.Count ?? 1))),
        _tape.Connect().Subscribe(
            new ActionObserver<ChangeSet<InstrumentSnapshot>>(
                changes => RecordStream(
                    "Connect() ChangeSet",
                    CacheAction.BatchOperation,
                    changes.Count))),
        _indexedSnapshots.Stream.Subscribe(
            new ActionObserver<CacheNotify<InstrumentSnapshot>>(
                notification => RecordStream(
                    "QuaternaryList",
                    notification.Action,
                    notification.Batch?.Count ?? 1))),
        _latestByInstrument.Stream.Subscribe(
            new ActionObserver<CacheNotify<KeyValuePair<int, InstrumentSnapshot>>>(
                notification => RecordStream(
                    "QuaternaryDictionary",
                    notification.Action,
                    notification.Batch?.Count ?? 1)))
    ];
    ...
    private void ToggleRunning()
    {
        ...
        _ = Interlocked.Exchange(ref _uiFrames, 0);
    ...
    private void Reset()
    {
        ...
        _ = Interlocked.Exchange(ref _windowEvents, 0);
        _ = Interlocked.Exchange(ref _uiFrames, 0);
        _lastWindowEvents = 0;
        _rateWindow.Restart();
        EventsPerSecond = 0;
    ...
    private void ApplyFrame(MarketFrame frame)
    {
        ...
        if (_indexedSnapshots.Count > IndexedRetention)
        {
            ...
            _ = _indexedSnapshots.RemoveMany(snapshot => snapshot.Sequence < cutoff);
        }
        ...
        _ = Interlocked.Add(ref _windowEvents, frame.EventCount);
    ...
    private void ApplyUiFrame(MarketFrame frame)
    {
        ...
        _ = Interlocked.Increment(ref _uiFrames);
    ...
    private void UpdateMatrix(InstrumentSnapshot[] snapshots)
    {
        for (var row = 0; row < MatrixRowCount; row++)
        {
            ...
            for (var column = 0; column < values.Length; column++)
            {
                ...
                if (column > 0)
                {
                    _ = formattedValues.Append("  ");
                }

                _ = formattedValues.Append(CultureInfo.CurrentCulture, $"{price:N2}");
            }
    ...
    private void RecordStream(string source, CacheAction action, int count)
    {
        ...
        var item = new StreamEvent(
            ...
            _timeProvider.GetLocalNow().ToString("HH:mm:ss.fff"),
            source,
            action.ToString(),
            count,
            detail);
