...
public sealed class QuadDictionary<TKey, TValue> : IDisposable, IEnumerable<KeyValuePair<TKey, TValue>>, IQuad<KeyValuePair<TKey, TValue>>
{
    ...
    public QuadDictionary(IEqualityComparer<TKey>? comparer = null)
    {
        ...
        Changes = Observable.FromEventPattern<ChangeSet<KeyValuePair<TKey, TValue>>>(
                h => ItemChanged += h,
                h => ItemChanged -= h)
    ...
    public event EventHandler<ChangeSet<KeyValuePair<TKey, TValue>>>? ItemChanged;
    ...
    public IObservable<ChangeSet<KeyValuePair<TKey, TValue>>> Changes { get; }
    ...
    public TValue this[TKey key]
    {
        ...
        set
        {
            ...
            valueRef = value;
            ItemChanged?.Invoke(this, new ChangeSet<KeyValuePair<TKey, TValue>>(Change<KeyValuePair<TKey, TValue>>.CreateAdd(new KeyValuePair<TKey, TValue>(key, value))));
        }
    }
    ...
    public bool Remove(TKey key, [MaybeNullWhen(false)] out TValue value)
    {
        ...
        while (index >= 0)
        {
            ...
            if (entry.HashCode == hashCode && _comparer.Equals(entry.Key, key))
            {
                ...
                ItemChanged?.Invoke(this, new ChangeSet<KeyValuePair<TKey, TValue>>(Change<KeyValuePair<TKey, TValue>>.CreateRemove(new KeyValuePair<TKey, TValue>(key, value))));
                return true;
            }
    ...
    public void Clear()
    {
        if (_entryIndex > 0)
        {
            ...
            _freeList = -1;
            _freeCount = 0;
            ItemChanged?.Invoke(this, new ChangeSet<KeyValuePair<TKey, TValue>>(new Change<KeyValuePair<TKey, TValue>>(ChangeReason.Clear, default)));
        }
    }
    ...
    private ref TValue? AddNewEntry(TKey key, uint hashCode, ref int bucket)
    {
        ...
        bucket = index + 1;
        ItemChanged?.Invoke(this, new ChangeSet<KeyValuePair<TKey, TValue>>(Change<KeyValuePair<TKey, TValue>>.CreateAdd(new KeyValuePair<TKey, TValue>(key, newEntry.Value!))));
