...
{
    /// <summary>The number of instruments generated by the engine.</summary>
    private const int InstrumentCount = 40;

    /// <summary>The number of raw events generated for a full test frame.</summary>
    private const int RawEventCount = 1_000;

    /// <summary>The expected number of diagnostic samples in a frame.</summary>
    private const int SampleCount = 12;

    /// <summary>The minimum supported target event rate.</summary>
    private const int MinimumRate = 1_000;

    /// <summary>The maximum supported target event rate.</summary>
    private const int MaximumRate = 100_000;

    /// <summary>The number of frames generated before validating price stability.</summary>
    private const int PriceStabilityFrameCount = 20;

    /// <summary>The number of events generated after resetting the engine.</summary>
    private const int PostResetEventCount = 250;

    /// <summary>The target event rate used by the continuous producer test.</summary>
    private const int ContinuousTargetRate = 8_000;

    /// <summary>The expected number of events in a continuously produced frame.</summary>
    private const int ExpectedContinuousFrameEvents = 1_000;

    /// <summary>The exclusive lower price bound expected after repeated generation.</summary>
    private const double MinimumExpectedPrice = 50D;

    /// <summary>The exclusive upper price bound expected after repeated generation.</summary>
    private const double MaximumExpectedPrice = 250D;

    /// <summary>The maximum wait for the continuous producer to publish a frame.</summary>
    ...
    public async Task GeneratedPricesRemainWithinRealisticBounds()
    {
        ...
        await Assert.That(Array.TrueForAll(frame.Snapshots, static snapshot => snapshot.Price is > MinimumExpectedPrice and < MaximumExpectedPrice)).IsTrue();
        await Assert.That(Array.Exists(frame.Snapshots, static snapshot => snapshot.IsAlert)).IsTrue();
        await Assert.That(Array.Exists(frame.Snapshots, static snapshot => !snapshot.IsAlert)).IsTrue();
    }
    ...
    {
        using var engine = new LiveDataEngine { TargetEventsPerSecond = 1 };
    ...
    {
        using var engine = new LiveDataEngine { TargetEventsPerSecond = ContinuousTargetRate };
